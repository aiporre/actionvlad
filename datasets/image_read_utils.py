...
def _read_from_disk_spatial(fpath, nframes, num_samples=25, start_frame=0,
    ...
    if step is None:
      if num_samples == 1:
          step = tf.random.uniform([1], 0, nframes, dtype='int32')[0]
      else:
          step = tf.cast((duration-tf.constant(1)) /
                         (tf.constant(num_samples-1)), 'int32')
    allimgs = []
    with tf.compat.v1.variable_scope('read_rgb_video'):
        for i in range(num_samples):
            if num_samples == 1:
                i = 1  # so that the random step value can be used
            with tf.compat.v1.variable_scope('read_rgb_image'):
                prefix = file_prefix + '_' if file_prefix else ''
                impath = tf.strings.join([
                    ...
                    tf.constant('.jpg')])
                img_str = tf.io.read_file(impath)
...
def _read_from_disk_temporal(
    ...
    allimgs = []
    with tf.compat.v1.variable_scope('read_flow_video'):
        for i in range(num_samples):
            if num_samples == 1:
                i = 1  # so that the random step value can be used
            with tf.compat.v1.variable_scope('read_flow_image'):
              flow_img = []
              for j in range(optical_flow_frames):
                with tf.compat.v1.variable_scope('read_flow_channels'):
                  for dr in ['x', 'y']:
                    prefix = file_prefix + '_' if file_prefix else ''
                    impath = tf.strings.join([
                        tf.constant(dataset_dir + '/'),
...
def decode_rgb(img_str):
  with tf.compat.v1.variable_scope('decode_rgb_frame'):
    ...
    img = tf.image.resize(img, [IM_HT, IM_WD])
  return [img]
...
def decode_flow(img_str):
  ...
  with tf.compat.v1.variable_scope('decode_flow_frame'):
...
def _decode_from_string(img_str, modality):
  if modality == 'rgb':
    ...
    with tf.compat.v1.name_scope('decode_rgbNflow'):
