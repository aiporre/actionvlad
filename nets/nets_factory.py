...
def get_network_fn(
    ...
    def network_fn(images, pool_type=None,
        ...
        for sid, images in enumerate(images_sets):
            with tf.compat.v1.variable_scope('stream%d' % sid):
                ...
                if pool_type in ['netvlad', 'avg-conv', 'max-conv']:
                    ...
                    if pool_type == 'netvlad':
                        ...
                        end_points[tf.compat.v1.get_variable_scope().name + '/netvlad'] = net
                        end_points.update(netvlad_end_points)
                    elif pool_type == 'avg-conv':
                        net = pooling.pool_conv(last_conv, batch_size, 'avg')
                        end_points[tf.compat.v1.get_variable_scope().name + '/avg-conv'] = net
                    elif pool_type == 'max-conv':
                        net = pooling.pool_conv(last_conv, batch_size, 'max')
                        end_points[tf.compat.v1.get_variable_scope().name + '/max-conv'] = net
                    if batch_norm:
                        with tf.compat.v1.variable_scope('pooled-batch-norm'):
                            net = slim.batch_norm(net, is_training=is_training)

                    if classifier_type is not None and classifier_type != 'None':
                        print(
                            'Dropout is not being applied to the model to be consistent with original release of the code. '
                    ...
                    if classifier_type == 'linear':
                        ...
                        with tf.compat.v1.variable_scope('classifier'):
                            ...
                            net = slim.fully_connected(
                                ...
                                normalizer_fn=None,
                                scope='logits-2')
                    end_points[tf.compat.v1.get_variable_scope().name + '/logits'] = net
                elif pool_type in ['avg', 'avg-after-softmax']:
                    if pool_type == 'avg-after-softmax':
                        net = tf.nn.softmax(net)
                    video_frames = tf.split(0, num_image_sets, net)
                    net = tf.concat(0, [tf.reduce_mean(input_tensor=el, axis=0, keepdims=True)
        ...
        if stream_pool_type == 'concat-netvlad':
            ...
            all_end_points[-1]['wtd-avg-pool-logits'] = net
                ...
                stream_pool_type == 'one-bag-and-netvlad':
            with tf.compat.v1.variable_scope(stream_pool_type):
                if stream_pool_type == 'one-bag-and-netvlad':
                    net = tf.concat(1, all_out_nets)
                else:
                    net = tf.concat(3, all_out_nets)
                end_points[tf.compat.v1.get_variable_scope().name + '/concat-last-conv'] = net
                net, netvlad_end_points = pooling.netvlad(
                    net, batch_size, 0.0, netvlad_initCenters=netvlad_centers[0])
                end_points[tf.compat.v1.get_variable_scope().name + '/concat-last-conv-netvlad'] = net
                end_points.update(netvlad_end_points)
